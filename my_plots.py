...
def top_names_plot(df, year=2000, n=10, width=800, height=600, variable='count'):
    ...
    fig = px.bar(df, x='name', y=variable, color='sex',
                ...
                color_discrete_map=color_map)  # Add custom hover data
...
def one_hit_wonders(ohw_data, year=1977):
    ...
    if ohw_year.empty:
        return pd.DataFrame({'Message': [f"No one-hit wonders found in {year}"]})
    ...
    for sex in ['F', 'M']:
        filtered = common_one_hit_wonders[common_one_hit_wonders['sex'] == sex]
        if not filtered.empty:
            most_common = filtered.sort_values(by='count', ascending=False).iloc[0]
            summary_data['Sex'].append(sex)
            summary_data['Most Common Name'].append(most_common['name'])
            summary_data['Occurrences'].append(most_common['count'])
            summary_data['Total One-Hit Wonders'].append(one_hit_wonder_counts.get(sex, 0))
        else:
            summary_data['Sex'].append(sex)
            summary_data['Most Common Name'].append("N/A")
            summary_data['Occurrences'].append(0)
            summary_data['Total One-Hit Wonders'].append(0)
